...
subroutine water_vapor_self_continuum(jrad, wk, wtot, grid, &
  ...
  real(kind=real64), dimension(:), allocatable, intent(inout) :: csh2o !< [cm3].
  real(kind=real64), dimension(:), intent(inout) :: absrb !< Extinction [cm-1].
  real(kind=real64), dimension(:), allocatable, intent(inout) :: cself !< Extinction [cm-1].

  integer :: j, nptc, ist, last
  real(kind=real64) :: h2o_fac, rself, v1c, v2c, dvc, v1ss, v2ss, vj, sh2o, tfac
  real(kind=real64), dimension(:), allocatable :: sh2ot0, sh2ot1
  ...
  if (grid%xn .gt. -20.0_real64 .and. grid%x0 .lt. 20000._real64) then
    ...
    allocate(cself(nptc), csh2o(nptc))
    do j = 1, nptc
...
subroutine water_vapor_foreign_continuum(icflg, jrad, wk, wtot, grid, &
  ...
  real(kind=real64), dimension(:), allocatable, intent(inout) :: cself !< Extinction [cm-1].
  character(len=*), intent(in) :: path !< Path to dataset.
  real(kind=real64), dimension(:), allocatable, intent(inout) :: cfh2o !< [cm3].
  real(kind=real64), dimension(:), intent(inout) :: absrb !< Extinction [cm-1].
  ...
  real(kind=real64) :: f0, v0f1, hwsq1, beta1, c_1, c_2, beta2, v1c, v2c, dvc, v1ss, &
                       ...
                       rfrgn, rfrgn_aj, c_f
  real(kind=real64), dimension(:), allocatable :: fh2o, c, cfrgn_aj
  ...
  if (grid%xn .gt. -20.0_real64 .and. grid%x0 .lt. 20000._real64) then
    ...
    allocate(cfh2o(nptc), c(nptc), cfrgn_aj(nptc))
    do j = 1, nptc
      vj = v1c + dvc*real(j - 1) ![cm-1].
    ...
    if (icflg .eq. 1) then
      if (allocated(cself)) then
        do j = 1, nptc
          c(j) = cself(j) - cfrgn_aj(j) ![cm-1].
        enddo
        deallocate(cself)
      else
        c(:) = -1._real64*cfrgn_aj(:)
      endif
      call pre_xint(v1ss, v2ss, grid%x0, grid%dx, grid%n, ist, last)
      call xint(v1c, v2c, dvc, c, 1._real64, grid%x0, grid%dx, absrb, ist, last)
    endif
    deallocate(fh2o, c, cfrgn_aj)
  endif
