...
path_length = 1. ![cm].
call grid%construct(0._real64, 10000._real64, 2._real64) !Spectral grid.
...
enddo
call nc_check(nf90_put_var(ncid, varid, wavenumber(1:grid%n)))
call nc_check(nf90_def_var(ncid, "absrb", nf90_double, dimid(1:1), varid))
call nc_check(nf90_put_att(ncid, varid, "units", ""))
call nc_check(nf90_put_var(ncid, varid, absrb(1:grid%n)))
...
call nc_check(nf90_put_att(ncid, varid, "units", "cm-1"))
call nc_check(nf90_put_var(ncid, varid, wavenumber(1:h2o_grid%n)))
...
call nc_check(nf90_put_var(ncid, varid, csh2o(1:h2o_grid%n)))
...
call nc_check(nf90_put_var(ncid, varid, cfh2o(1:h2o_grid%n)))
...
call nc_check(nf90_put_var(ncid, varid, csh2or(1:h2o_grid%n)))
...
call nc_check(nf90_put_var(ncid, varid, cfh2or(1:h2o_grid%n)))
